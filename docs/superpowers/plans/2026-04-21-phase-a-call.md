...
- Current `main` already split the Liblinphone implementation across `src/yoyopod/communication/integrations/liblinphone/`, `src/yoyopod/communication/calling/backend_protocol.py`, `src/yoyopod/communication/calling/liblinphone_backend.py`, and `src/yoyopod/communication/calling/mock_backend.py`.
- When this plan says "move `communication/calling/backend.py`", read that as "retire the compatibility wrapper in `communication/calling/backend.py` after the real implementation has moved".
- The native shim source currently lives under `src/yoyopod/communication/integrations/liblinphone/native/`.
- `src/yoyopod/communication/integrations/liblinphone_binding/` is already a compatibility alias layer; migrate from the real `integrations/liblinphone/` package, then delete the alias once imports are updated.
...
- `src/yoyopod/backends/voip/liblinphone.py` (moved from `src/yoyopod/communication/integrations/liblinphone/backend.py`)
- `src/yoyopod/backends/voip/binding.py` (moved from `src/yoyopod/communication/integrations/liblinphone/binding.py`)
- `src/yoyopod/backends/voip/shim_native/` (moved from `src/yoyopod/communication/integrations/liblinphone/native/` — keep structure)
...
- `src/yoyopod/communication/calling/backend.py` (compatibility wrapper removed)
- `src/yoyopod/communication/calling/liblinphone_backend.py` (compatibility alias removed)
- `src/yoyopod/communication/calling/mock_backend.py`
- `src/yoyopod/communication/calling/backend_protocol.py`
...
git mv src/yoyopod/communication/integrations/liblinphone/backend.py src/yoyopod/backends/voip/liblinphone.py
git mv src/yoyopod/communication/integrations/liblinphone/binding.py src/yoyopod/backends/voip/binding.py
git mv src/yoyopod/communication/calling/backend_protocol.py src/yoyopod/backends/voip/protocol.py
git mv src/yoyopod/communication/calling/mock_backend.py src/yoyopod/backends/voip/mock_backend.py
```

The native shim directory may contain C source; move the whole directory:

```bash
git mv src/yoyopod/communication/integrations/liblinphone/native src/yoyopod/backends/voip/shim_native
git rm -r src/yoyopod/communication/integrations/liblinphone_binding
```
...
```bash
grep -rn "from yoyopod.communication.calling.backend\|from yoyopod.communication.calling.liblinphone_backend\|from yoyopod.communication.calling.backend_protocol\|from yoyopod.communication.integrations.liblinphone\|from yoyopod.communication.models\|from yoyopod.communication import" src/ tests/
```

Rewrite to new paths:
- `yoyopod.communication.calling.backend` / `yoyopod.communication.calling.liblinphone_backend` → `yoyopod.backends.voip.liblinphone`
- `yoyopod.communication.calling.backend_protocol` → `yoyopod.backends.voip.protocol`
- `yoyopod.communication.integrations.liblinphone.*` → `yoyopod.backends.voip.*`
