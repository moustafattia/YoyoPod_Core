...
- The current display facade lives in `src/yoyopod/ui/display/manager.py`, not `facade.py`.
- Shared display contracts currently live in `src/yoyopod/ui/display/hal.py` and `contracts.py`.
- `src/yoyopod/ui/lvgl_binding/` currently exposes `binding.py` plus the native shim; there is no Python `backend.py` to move verbatim.
- The LVGL build command currently lives under `yoyopod_cli/build.py`, not `src/yoyopod/cli/build.py`.
...
- `src/yoyopod/backends/display/lvgl/backend.py` (new thin wrapper if a dedicated LVGL backend module is still needed after the move)
...
Open `yoyopod_cli/build.py` (or wherever `yoyopod build lvgl` is defined). Update the path it expects the native source at — from `src/yoyopod/ui/lvgl_binding/native/` to `src/yoyopod/backends/display/lvgl/native_shim/`.
...
```bash
# Current main uses manager.py + hal.py instead of facade.py. Extract the
# consumer-facing API from those files into the new backend module.
git mv src/yoyopod/ui/display/manager.py src/yoyopod/backends/display/api.py
