...
fn main() {
    ...
    let lvgl_source_dir = normalize_cmake_path(lvgl_source_dir.canonicalize().unwrap_or_else(
        |_| panic!("LVGL source directory not found: {}", lvgl_source_dir.display()),
    ));
    ...
    let link_dir = find_library_dir(&dst, library_file_names()).unwrap_or_else(|| {
        panic!(
